...
public class OptimisticLock {
	public static void main(String[] args) {
		...
		Thread sheik=new Thread(new CounsellingJob(university),"Sheik");
		Thread nithish=new Thread(new CounsellingJob(university),"Nithish");
		sheik.start();
		nithish.start();
	}
}
class CounsellingJob implements Runnable{
	...
	//	synchronized (university) {
		...
		if(Thread.currentThread().getName().equals("Sheik")) {
			university.table1();university.table2();
		}
		else if(Thread.currentThread().getName().equals("Nithish")){
			university.water();
		}
	//	}
	}
